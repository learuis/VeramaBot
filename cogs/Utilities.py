...
class Utilities(commands.Cog):
    ...
    @commands.dynamic_cooldown(custom_cooldown, type=commands.BucketType.user)
    @commands.check(checkChannel)
    async def iam(self, ctx, command: str = commands.parameter(default='who')):
        """- Map discord ID to character name

        Maps your discord ID to your character name for RCON command purposes.

        `v/iam who` -- queries the database for a record matching your discord ID

        `v/iam Name` -- writes an entry to the database matching your discord ID to the name provided.

        `v/iam delete` -- deletes the record in the database matching your discord ID.
        ...
        cur.execute(f'select * from AuthorMappings where discord_id = \'{ctx.message.author}\'')
        records = cur.fetchone()
        if records:
            if command == 'delete':
                cur.execute(f'delete from AuthorMappings where discord_id = \'{ctx.message.author}\'')
                con.commit()
                con.close()
                await ctx.send(f'Deleted record for {ctx.message.author}.')
                return
            elif command == 'who':
                discord_id, character_name = records
                await ctx.send(f'You are:\n`Discord ID: {discord_id} | Character Name: {character_name}`')
                con.close()
                return
            else:
                await ctx.send(f'Mapping record already exists!')
                con.close()
                return
        else:
            if command == 'delete' or command == 'who':
                await ctx.send(f'No record exists for {ctx.message.author}.')
                con.close()
                return
            else:
                cur.execute(
                    f'insert into AuthorMappings (discord_id, character_name) \
                      values (\'{ctx.message.author}\', \'{command}\')')
                con.commit()
                cur.execute(f'select * from AuthorMappings where discord_id =  \'{ctx.message.author}\'')
                records2 = cur.fetchone()
                if records2:
                    discord_id, character_name = records2
                    await ctx.send(f'Record created: `Discord ID: {discord_id} | Character Name: {character_name}`')
                    con.close()
                    return
                else:
